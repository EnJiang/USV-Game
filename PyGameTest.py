...
class MyUSV(OneStepUSV):
    ...
    def attackDecisionAlgorithm(self):
        ...
        distanceUp = self._euclideanDistance(
            targetX, targetY, self.x, (self.y - 1))
        distanceDown = self._euclideanDistance(
            targetX, targetY, self.x, (self.y + 1))
        distanceLeft = self._euclideanDistance(
            targetX, targetY, (self.x - 1), self.y)
        distanceRight = self._euclideanDistance(
            targetX, targetY, (self.x + 1), self.y)
        ...
        for distance in distances:
            ...
            if(self.isDecisionLegal(decisionX, decisionY)):
                ...
                finalDecision["angularSpeed"] = abs(
                    angularToBe - self.direction)
                break
