...
from usv import MyContinueUSV, MyContinueDynamicsUSV3partWorld
...
if __name__ == '__main__':
    ...
    '''开始游戏'''
    test_map = MyContinueObsMap(30, 30)
    test_map.set_target(5.0,25.0)
    ...
    if dynamicsSwitch == True:
        ...
        test_friendly_ship = MyContinueDynamicsUSV3partWorld(uid=0, x=25.0, y=5.0, env=test_map, envDisturb=envDisturbSwitch,FTListValue=FTListValue)

    else:
        test_friendly_ship = MyContinueUSV(uid=0, x=25.0, y=5.0, env=test_map)
    ...
    obs1 = CircleObstacle(uid=0, x=15.0, y=15.0, radius=1, env=test_map);test_map.addobs(obs1)
    obs2 = CircleObstacle(uid=1, x=8.0, y=8.0, radius=1, env=test_map);test_map.addobs(obs2)
    obs3 = CircleObstacle(uid=2, x=4.0, y=16.0, radius=1, env=test_map);test_map.addobs(obs3)
    obs4 = CircleObstacle(uid=3, x=15.0, y=5.0, radius=1, env=test_map);test_map.addobs(obs4)


    obs5 = CircleObstacle(uid=4, x=21.0, y=20.0, radius=1, env=test_map);test_map.addobs(obs5)
    obs6 = CircleObstacle(uid=5, x=28.0, y=9.0, radius=1, env=test_map);test_map.addobs(obs6)
    obs7 = CircleObstacle(uid=6, x=13.0, y=28.0, radius=1, env=test_map);test_map.addobs(obs7)
